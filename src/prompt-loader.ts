 ...
 */
function createZodSchema(param: PromptParameter): ZodTypeAny {
 ...
 */
async function loadPromptFile(filePath: string): Promise<PromptDefinition | null> {
  try {
    const content = await readFile(filePath, "utf-8");
    const promptDef = JSON.parse(content) as PromptDefinition;
 ...
 */
async function loadPromptsFromDirectory(dirPath: string): Promise<PromptDefinition[]> {
 ...
 */
function registerPrompt(server: McpServer, promptDef: PromptDefinition): void {
 ...
 */
async function loadAllPrompts(): Promise<PromptDefinition[]> {
 ...
 */
async function setupHotReload(server: McpServer): Promise<void> {
  ...
  const watchDirectory = async (dirPath: string) => {
    try {
      ...
      for await (const event of watcher) {
        if (!event.filename || !event.filename.endsWith(".json")) continue;
 ...
 */
export async function loadAndRegisterExternalPrompts(server: McpServer): Promise<number> {
  // Initial load
  const prompts = await loadAllPrompts();
