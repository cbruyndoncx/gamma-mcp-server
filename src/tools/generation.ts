...
export function registerGenerateTool(server: McpServer): void {
  server.tool(
    "generate",
    "Create a presentation, document, webpage or social post from scratch with the Gamma API. Waits for the generation to finish and returns a link, unless waitForCompletion is false.",
    {
      ...
      textMode: z
        ...
        .optional()
        .describe(
          `How to treat inputText (${GAMMA_TEXT_MODES.join(" | ")}). ` +
      ...
      cardSplit: z
        ...
        .optional()
        .describe(
          `How content is divided (${GAMMA_CARD_SPLIT.join(" | ")}). ` +
      ...
      exportAs: z
        ...
        .optional()
        .describe(
          `Also export the result (${GAMMA_EXPORT_FORMATS.join(" | ")}). One format per generation. ` +
      ...
      waitForCompletion: z
        ...
        .optional()
        .describe(
          "Wait for the generation to finish before returning (default true). " +
...
export function registerGenerateMultiPageTool(server: McpServer): void {
  server.tool(
    "generate_multi_page_gamma",
    "Create one Gamma containing several distinct pages under a single URL - for sales rooms, proposal sites, onboarding portals and microsites. Prefer 'generate' for a single presentation, document, webpage or social post. Cannot edit an existing Gamma.",
    {
      pages: z
        .array(pageSchema)
        .min(1)
      ...
      exportAs: z
        ...
        .optional()
        .describe(
          `Export each page (${GAMMA_EXPORT_FORMATS.join(" | ")}). ` +
    ...
    async ({ waitForCompletion, ...params }) => {
      const result = await generateMultiPage(
        params as GammaMultiPageParams,
...
export function registerGenerateFromTemplateTool(server: McpServer): void {
  server.tool(
    "generate_from_template",
    "Create a new gamma by adapting, remixing or transforming an existing one used as a template. The template's structure is preserved unless the prompt asks otherwise. Use get_gammas with type 'template' to find a template ID.",
    {
      gammaId: z
        .string()
        .describe(
          "File ID of the template gamma. This is the API file ID (often prefixed 'g_'), " +
      ...
      imageOptions: z
        ...
        .optional()
        .describe(
          "Image overrides for templates that use AI-generated images. " +
    ...
    async ({ waitForCompletion, ...params }) => {
      const result = await generateFromTemplate(
        params as GammaFromTemplateParams,
        waitForCompletion !== false
      );
