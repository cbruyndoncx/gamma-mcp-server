...
export function registerGetGenerationStatusTool(server: McpServer): void {
  server.tool(
    "get_generation_status",
    `Check the status of a generation started by generate, generate_multi_page_gamma or generate_from_template. Returns status, gammaUrl, gammaId, exportUrl and credit usage. Poll until status is completed or failed. ${EXPORT_URL_CAVEAT}`,
    {
...
export function registerDownloadExportTool(server: McpServer): void {
  server.tool(
    "download_export",
    `Download a completed generation's export to the machine running this MCP server, and return the local file path. The generation must have been created with exportAs set. Set GAMMA_DOWNLOAD_DIR to choose the destination (defaults to /tmp). ${EXPORT_URL_CAVEAT}`,
    {
    ...
    async ({ generationId }) => {
      try {
        ...
        if (!result.exportUrl) {
          return textResult(
            `Generation ${generationId} has no export. Its status is '${result.status}'. ` +
