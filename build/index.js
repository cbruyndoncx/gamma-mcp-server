...
async function generatePresentation(params) {
    try {
        ...
        const timeoutMs = 60000; // 60s total timeout
        ...
        while (Date.now() - start < timeoutMs) {
            ...
            if (status === "completed" || status === "succeeded") {
                ...
                if (possible)
                    return { url: possible, error: null };
...
server.tool("generate-presentation", "Generate a presentation using the Gamma API. The response will include a link to the generated presentation in the 'text' field. Always include the link in the response when it's available. Do your best to show a preview or a link preview or some sense of the content to the user in the response.", {
    inputText: z.string().describe("The topic or prompt for the presentation."),
    tone: z
        .string()
        .optional()
        .describe("The tone of the presentation (e.g. 'humorous and sarcastic')."),
    audience: z
        .string()
        .optional()
