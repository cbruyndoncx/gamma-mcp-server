...
export async function createImageGeneration(params) {
    ...
    if (params.sizePreset)
        body.sizePreset = params.sizePreset;
    if (params.themeId)
...
/** GET /images/{id} */
export async function getImageGenerationStatus(imageGenerationId) {
    return apiRequest(`/images/${imageGenerationId}`);
}
/** POST /images/media/{savedMediaId}/archive - idempotent. */
export async function archiveImage(savedMediaId) {
    return apiRequest(`/images/media/${savedMediaId}/archive`, { method: "POST" });
}
/**
 ...
 */
export async function generateImageAndWait(params) {
