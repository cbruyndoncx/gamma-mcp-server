 ...
 */
import { z } from "zod";
import { searchGammas, searchTemplates, getGamma, getGammaComments, exportAndWait, getExportStatus, archiveGamma, deleteGamma, } from "../api/management.js";
import { textResult, jsonResult } from "./format.js";
import { GAMMA_EXPORT_FORMATS } from "../constants.js";
const GAMMA_ID_DESC = "Gamma file ID or a full gamma.app/docs/... URL. The URL form is accepted and the ID extracted; " +
    "note the API wants the file ID, not the human-readable slug.";
export function registerGetGammasTool(server) {
    server.tool("get_gammas", "Browse or search existing gammas and templates. Use type 'template' to find a template ID for generate_from_template.", {
        ...
        try {
            ...
            if (err?.statusCode === 403) {
                return textResult("Search is not enabled for this workspace yet. It is rolling out gradually - " +
...
export function registerReadGammaTool(server) {
    server.tool("read_gamma", "Read an existing Gamma's metadata: title, type, URL, thumbnail, description, author and timestamps. " +
...
export function registerGetGammaCommentsTool(server) {
    server.tool("get_gamma_comments", "List comment threads and replies on a Gamma, oldest activity first. Read-only, cursor-paginated.", {
        gammaIdOrUrl: z.string().describe(GAMMA_ID_DESC),
...
function renderExportStatus(result) {
    ...
    if (result.status === "failed") {
        ...
        const hint = reason === "deck_too_large"
...
export function registerExportGammaTool(server) {
    server.tool("export_gamma", "Export an existing gamma to a downloadable file. Waits for the export to finish. The connected user needs view access, and archived gammas cannot be exported.", {
        gammaIdOrUrl: z.string().describe(GAMMA_ID_DESC),
        exportAs: z
            .enum(GAMMA_EXPORT_FORMATS)
            .describe(`Format (${GAMMA_EXPORT_FORMATS.join(" | ")}). 'png' returns a .zip with one PNG per card.`),
    }, async ({ gammaIdOrUrl, exportAs }) => {
        try {
            const result = await exportAndWait(gammaIdOrUrl, exportAs);
...
export function registerGetExportStatusTool(server) {
    server.tool("get_export_status", "Check an export started by export_gamma and get the download link once ready.", {
...
export function registerArchiveGammaTool(server) {
    server.tool("archive_gamma", "Archive a Gamma, removing it from the active workspace. Idempotent. Requires edit permission. Not available on Gamma's official MCP server.", {
        gammaIdOrUrl: z.string().describe(GAMMA_ID_DESC),
    }, async ({ gammaIdOrUrl }) => {
        try {
            ...
            if (err?.statusCode === 403) {
                return textResult(`Access denied archiving ${gammaIdOrUrl}. Common causes: the ID is a web app URL slug ` +
...
export function registerDeleteGammaTool(server) {
    server.tool("delete_gamma", "PERMANENTLY DELETE a Gamma. This cannot be undone. Requires a workspace admin role on the API key's workspace. " +
        ...
        try {
            ...
            if (err?.statusCode === 403) {
                return textResult(`Access denied. DELETE requires a workspace admin role on the API key's workspace.`);
            }
