 ...
 */
import { z } from "zod";
import { generateImageAndWait, getImageGenerationStatus, archiveImage, } from "../api/images.js";
import { textResult } from "./format.js";
import { GAMMA_IMAGE_TYPES, GAMMA_IMAGE_SIZE_PRESETS } from "../constants.js";
function renderImageStatus(result) {
    const lines = [];
    if (result.status === "completed" && result.image) {
        ...
        lines.push(`Image generation is still pending (id=${result.imageGenerationId}).`, `Poll get_image_generation_status with this id.`);
    }
...
export function registerGenerateImageTool(server) {
    server.tool("generate_image", "Generate one standalone on-brand image from a text prompt - an illustration, scene, photo or abstract graphic on its own, not a gamma. Charges credits.", {
        prompt: z
            .string()
            .min(1)
            .max(5_000)
            .describe("What to generate. Put background treatments here too, e.g. 'on a white background'."),
        type: z
            .enum(GAMMA_IMAGE_TYPES)
            .optional()
            .describe(`Visual style (${GAMMA_IMAGE_TYPES.join(" | ")}). Defaults to illustration. ` +
            `'scene' uses a curated style that ignores theme colours.`),
        sizePreset: z
            .enum(GAMMA_IMAGE_SIZE_PRESETS)
            .optional()
            .describe(`Aspect ratio (${GAMMA_IMAGE_SIZE_PRESETS.join(" | ")}). Defaults to social-square. ` +
        ...
        referenceImages: z
            ...
            .max(4)
            .optional()
            .describe("Up to 4 reference images whose subject should appear in the result. " +
...
export function registerGetImageGenerationStatusTool(server) {
    server.tool("get_image_generation_status", "Check an image generation started by generate_image and get the image URL once ready. Poll every few seconds rather than in a tight loop. Do not call generate_image again to check progress - that starts a new, separately billed generation.", {
...
export function registerArchiveImageTool(server) {
    server.tool("archive_image", "Archive an item in the media library. Idempotent - archiving an already archived item succeeds. Not available on Gamma's official MCP server.", {
